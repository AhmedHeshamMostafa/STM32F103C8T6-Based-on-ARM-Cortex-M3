...
SPICfg_t SPIConfigStruct = {SLAVE, EIGHTBIT_FRAME, LSB_FIRST, IDLE_LOW, SAMPLE_1ST_EDGE, FPCLK_OVER2, SPI_CH1};
...
int main(void)
{
	...
	MGPIO_u8CfgOutPin (PA, PIN0, OUT_PUSH_PULL, OUT_HIGH_SPEED);

	MGPIO_u8CfgPinMode(PA, PIN7, ALTERNATE_FN);
	MGPIO_u8CfgPinMode(PA, PIN6, ALTERNATE_FN);
	MGPIO_u8CfgPinMode(PA, PIN5, ALTERNATE_FN);
	MGPIO_u8CfgPinMode(PA, PIN4, ALTERNATE_FN);

	MGPIO_u8SetAF(PA, PIN7, SPI1_TO_4);
	MGPIO_u8SetAF(PA, PIN6, SPI1_TO_4);
	MGPIO_u8SetAF(PA, PIN5, SPI1_TO_4);
	MGPIO_u8SetAF(PA, PIN4, SPI1_TO_4);

	MRCC_BoolEnablePerClk(SPI1);

	MSPI_BoolInit(&SPIConfigStruct);
	MSPI_vEnableSPI(&SPIConfigStruct);



	while(1)
	{
		MSPI_vTransceiveFrame(&SPIConfigStruct,G_DataSent, &G_DataRecieved);

		if(G_DataRecieved == 0x11)
		{
